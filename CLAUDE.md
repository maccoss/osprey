...
3. **Reconciliation** uses first-pass protein q-values as an optional rescue gate in `compute_consensus_rts()`. A detection qualifies for consensus only if its own `run_precursor_qvalue <= consensus_fdr` (a hard precondition); the protein rescue then upgrades entries whose peptide-level q-value is borderline (OR `run_protein_qvalue <= protein_fdr_threshold`). Protein FDR cannot rescue detections with poor precursor-level evidence because the consensus RT is driven by each entry's own `apex_rt`. See `docs/10-cross-run-reconciliation.md#step-1-consensus-peptide-selection`.
...
**CRITICAL: Ranking score is raw SVM discriminant.** During development we tried peptide q-value and peptide PEP as alternatives; both collapsed the decoy null distribution because losing decoys have q=1.0 or PEP clamped to ~1.0 (see `docs/07-fdr-control.md` for the full history). SVM discriminant is the correct choice: every entry has a well-defined score on the same scale.

**Protein FDR calibration depends on correct peak selection.** The Gaussian RT penalty in CWT peak selection (`run_search()`) is critical for protein FDR calibration. Without it, wrong-peak interferers inflate target SVM scores, eliminating the target-decoy overlap picked-protein needs (collapsed to 2/6099 decoy winners on Stellar). With the RT penalty, Stellar produces 57/6441 decoy winners (0.89% decoy rate, well calibrated at 1% FDR). If protein FDR calibration degrades on a dataset, check peak selection quality first (the decoy-winner fraction is the diagnostic).

**Remaining concern: decoy asymmetry.** Target library entries use AI-predicted spectra and RTs (Carafe). `DecoyGenerator::Reverse` produces reversed peptide sequences but keeps the target's RT and does a mechanical reversal of fragments. This asymmetry lets the SVM exploit features that distinguish targets from decoys beyond random matching. Re-predicting decoy spectra and RTs from the same model would make the two pools more comparable. This is deferred to future work.
...
- **Reconciliation RT tolerance: within-peptide MAD, not cross-peptide calibration MAD.** `plan_reconciliation` now derives `rt_tolerance` from the global median of per-peptide apex-RT MADs in library space (computed in `compute_consensus_rts` for peptides with ≥ 3 detections). Typical Stellar tolerance drops from ~0.3 min (cross-peptide calibration MAD) to ~0.1 min (within-peptide reproducibility floored). The per-file calibration MAD is retained as a safety ceiling. See `docs/10-cross-run-reconciliation.md`.
- **Consensus weighting: sigmoid(SVM score), not coelution_sum.** `compute_consensus_rts` now uses `sigmoid(score)` as the weighted-median weight for both `consensus_library_rt` and `median_peak_width`. A wrong-peak detection with negative SVM score gets weight ~0.02, cannot poison the median; strong detections with positive score dominate. The `coelution_sum > 0` sanity filter is kept. Regression tests: `test_consensus_weighting_downweights_negative_score_detections`, `test_consensus_rejects_low_precursor_q_despite_protein_rescue`.
- **Consensus qualification gate: precursor q-value is a hard precondition.** `compute_consensus_rts` now requires `run_precursor_qvalue <= consensus_fdr` for every contributing target detection, BEFORE considering peptide-q or protein-rescue. Protein FDR can only upgrade borderline peptide-level evidence; it cannot rescue detections with poor precursor-level evidence. Fixes the case where strong proteins' wrong-peak charge-state detections poisoned the charge-agnostic consensus RT.
- **`OSPREY_TRACE_PEPTIDE` env-var for per-peptide diagnostic trace.** Set to a modified_sequence (comma-separated for multiple) to emit `[trace]` log lines at first-pass CWT scoring, `compute_consensus_rts`, `plan_reconciliation` (including tolerance derivation + stored CWT candidates), `identify_gap_fill_targets`, and both FDR passes. Matches paired decoys (`DECOY_<target>`) automatically. Zero overhead when unset (OnceLock-guarded). See `docs/17-peptide-trace.md` for format and typical workflows.
