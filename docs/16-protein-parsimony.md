...
> **Why SVM discriminant and not peptide q-value or PEP?** During development we tried both alternatives and both collapsed the decoy null distribution — q-value because 99% of decoys have `q = 1.0` after losing peptide-level TDC, and PEP because Osprey's `PepEstimator` is fit on TDC winners only and clamps out-of-range scores to `bins[0] ≈ 1.0`. Raw SVM scores avoid both failure modes: every entry has a score on the same scale. Protein FDR calibration also depends critically on correct peak selection — the [Gaussian RT penalty](../docs/10-cross-run-reconciliation.md#rt-penalized-peak-selection) in CWT peak selection prevents wrong-peak interferers from inflating target SVM scores, which restores the target-decoy overlap picked-protein needs. On Stellar 3-file HeLa with the RT penalty: 57/6441 decoy winners (0.89% decoy rate, well calibrated). See `docs/07-fdr-control.md` for the full history.
