...
mod tests {
    ...
    /// Verifies that BinnedLibrary correctly indexes entries and maps between IDs and row indices.
    #[test]
    ...
    /// Verifies that extracting a design matrix subset returns correct IDs and normalized columns summing to 1.0.
    #[test]
    ...
    /// Verifies that OptimizedSolver produces all non-negative coefficients with a reasonable residual.
    #[test]
    ...
    /// Verifies that BinnedSpectraCache returns cached results on subsequent lookups for the same scan.
    #[test]
    ...
    /// Verifies that explained_variance returns 0.8 when the residual is 20% of the observed norm.
    #[test]
