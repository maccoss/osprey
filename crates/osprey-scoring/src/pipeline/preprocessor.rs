...
//! as they stream in, preparing them for batch scoring.
...
pub struct PreprocessingConfig {
    /// Number of bins for scoring (default: 2000)
    pub num_bins: usize,
...
/// A spectrum that has been preprocessed for scoring
///
/// Contains both LibCosine (SMZ) and XCorr preprocessed vectors,
/// plus metadata needed for window-based scoring.
#[derive(Debug, Clone)]
pub struct PreprocessedSpectrum {
    ...
    /// SMZ-preprocessed and L2-normalized vector for LibCosine scoring
    pub libcosine_vector: Array1<f64>,
    /// XCorr-preprocessed vector (windowed + flanking subtracted)
    pub xcorr_vector: Vec<f64>,
}

/// Worker that preprocesses spectra for batch scoring
#[derive(Debug, Clone)]
...
impl PreprocessingWorker {
    ...
    /// Preprocess a spectrum for both LibCosine and XCorr scoring
    ///
    /// This performs all preprocessing once so the result can be used
    /// for scoring against multiple library entries.
    pub fn process(&self, spectrum: &Spectrum) -> PreprocessedSpectrum {
        let libcosine_vector = self.preprocess_for_libcosine(spectrum);
    ...
    /// Preprocess spectrum for LibCosine scoring (SMZ + L2 normalization)
    fn preprocess_for_libcosine(&self, spectrum: &Spectrum) -> Array1<f64> {
        let mut binned = Array1::zeros(self.config.num_bins);

        // Bin with SMZ preprocessing: sqrt(intensity) × m/z²
        for (&mz, &intensity) in spectrum.mzs.iter().zip(spectrum.intensities.iter()) {
            let bin = ((mz - self.config.min_mz) / self.config.bin_width) as usize;
        ...
        // L2 normalize
        let norm: f64 = binned.iter().map(|x| x * x).sum::<f64>().sqrt();
        if norm > 1e-10 {
            binned /= norm;
        }
    ...
    fn preprocess_for_xcorr(&self, spectrum: &Spectrum) -> Vec<f64> {
        // Bin with sqrt transformation
        let mut binned = vec![0.0f64; self.config.num_bins];
        for (&mz, &intensity) in spectrum.mzs.iter().zip(spectrum.intensities.iter()) {
    ...
    fn apply_windowing_normalization(&self, binned: &[f64]) -> Vec<f64> {
    ...
    fn apply_sliding_window(&self, windowed: &[f64]) -> Vec<f64> {
        let offset = 75; // Comet default offset
        let mut result = vec![0.0f64; windowed.len()];
...
mod tests {
    ...
    fn test_preprocessing_worker() {
        ...
        assert_eq!(preprocessed.libcosine_vector.len(), worker.config.num_bins);
        assert_eq!(preprocessed.xcorr_vector.len(), worker.config.num_bins);

        // LibCosine vector should be L2 normalized (sum of squares ≈ 1)
        let l2_norm: f64 = preprocessed
            .libcosine_vector
            .iter()
            .map(|x| x * x)
            .sum::<f64>()
            .sqrt();
        assert!((l2_norm - 1.0).abs() < 0.001 || l2_norm < 0.001);
    }
    ...
    fn test_empty_spectrum() {
        ...
        assert_eq!(preprocessed.libcosine_vector.sum(), 0.0);
    }
}
