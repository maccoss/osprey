...
pub fn run_analysis(mut config: OspreyConfig) -> Result<()> {
    ...
    // Decoys: either supplied by the library (DIA-NN / EncyclopeDIA output
    // with `rev_` / `DECOY_` prefixes) or generated by Osprey from the targets.
    // `decoy_method = FromLibrary` is treated as a synonym for
    // `decoys_in_library = true` — historically it silently fell through to
    // Reverse generation, which was the bug behind v26.5.3's library-decoy
    ...
    if library_supplies_decoys {
        ...
        let decoy_method = match config.decoy_method {
            ...
            // FromLibrary is handled by the branch above; this arm is
