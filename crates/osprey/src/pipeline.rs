...
use osprey_chromatography::{PeakDetector, RTCalibration, RTCalibrator, RTCalibratorConfig};
use osprey_core::{
    BinConfig, DecoyMethod as CoreDecoyMethod, FeatureSet, LibraryEntry, OspreyConfig, OspreyError,
    RegressionResult, ResolutionMode, Result, Spectrum,
};
use osprey_fdr::{FdrController, MokapotRunner, PsmFeatures};
use osprey_io::{load_library, BlibWriter, MzmlReader};
use osprey_regression::{Binner, DesignMatrixBuilder, RidgeSolver};
use osprey_scoring::{DecoyGenerator, DecoyMethod, Enzyme, FeatureExtractor};
use rayon::prelude::*;
...
pub fn run_analysis(config: OspreyConfig) -> Result<Vec<RegressionResult>> {
    ...
    if !config.decoys_in_library {
        ...
        // Generate decoys (assuming trypsin digestion - most common)
        ...
        let decoys = generator.generate_all(&targets.into_iter().cloned().collect::<Vec<_>>())?;
        log::info!("Generated {} decoys from {} targets", decoys.len(), n_targets);
    ...
    let bin_config = match config.resolution_mode {
        ...
        ResolutionMode::HRAM { tolerance_ppm } => BinConfig::hram_ppm(tolerance_ppm),
        ResolutionMode::Auto => {
    ...
    for (file_idx, input_file) in config.input_files.iter().enumerate() {
        ...
        let (file_results, file_spectra, calibration_result) = process_file_with_calibration(
            input_file,
            &library,
            &library_by_mz,
            &matrix_builder,
...
///
/// Returns: (results, spectra, Option<(calibration, tolerance)>)
...
fn process_file_with_calibration(
...
) -> Result<(Vec<RegressionResult>, Vec<Spectrum>, Option<(RTCalibration, f64)>)> {
    ...
    if !config.rt_calibration.enabled {
        ...
        return Ok((results, spectra, None));
    }
    ...
    if let Some((cal, tol)) = existing_calibration {
        ...
        return Ok((results, spectra, None)); // Don't return calibration for subsequent files
    }

    // First file: Run calibration discovery with ALL peptides
    log::info!("First file: RT Calibration Discovery (using all peptides)");

    let calibration = run_calibration_discovery(
        &spectra,
        library,
        library_by_mz,
        matrix_builder,
    ...
    let (rt_tolerance, calibration_opt) = match calibration {
        Ok(cal) => {
            let stats = cal.stats();
            log::info!(
            ...
            let tolerance = stats.residual_std * config.rt_calibration.rt_tolerance_factor;
        ...
        Err(e) => {
            log::warn!(
                "RT calibration failed: {}. Using fallback tolerance: {:.2} min",
                e,
                config.rt_calibration.fallback_rt_tolerance
            );
            (config.rt_calibration.fallback_rt_tolerance, None)
        }
    };
    ...
    Ok((results, spectra, calibration_to_share))
}

/// Run RT calibration discovery phase using LibCosine scoring (pyXcorrDIA style)
///
...
///
/// Process:
/// 1. For each target peptide: find max LibCosine score and RT
/// 2. For each decoy peptide: find max LibCosine score and RT
/// 3. Target-decoy competition → q-values
/// 4. Use only peptides at 1% FDR for calibration
/// 5. Fit LOESS on (library_RT, measured_RT) pairs
fn run_calibration_discovery(
    ...
    let entries_with_fragments: Vec<(usize, &LibraryEntry)> = library
        .iter()
        .enumerate()
        .filter(|(_, e)| !e.fragments.is_empty())
        .collect();

    let n_targets = entries_with_fragments.iter().filter(|(_, e)| !e.is_decoy).count();
    let n_decoys = entries_with_fragments.iter().filter(|(_, e)| e.is_decoy).count();
    ...
    // Set up progress bar
    let pb = ProgressBar::new(entries_with_fragments.len() as u64);
    pb.set_style(
        ProgressStyle::default_bar()
            .template(
                "{spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} (RT calibration)",
            )
            .unwrap()
            .progress_chars("#>-"),
    );

    // Create spectral scorer
    let scorer = SpectralScorer::new();

    // For each library peptide (target AND decoy), find the best matching spectrum
    // Returns: (library_rt, measured_rt, lib_cosine_score, is_decoy)
    let all_scores: Vec<(f64, f64, f64, bool)> = entries_with_fragments
        .par_iter()
        .filter_map(|(_, entry)| {
            let library_rt = entry.retention_time;

            // Find spectra where:
            // 1. Precursor is within isolation window
            // 2. RT is within initial tolerance of library RT
            ...
            for spectrum in spectra {
                ...
                // Check RT tolerance
                if (spectrum.retention_time - library_rt).abs() > initial_tolerance {
                    continue;
                }
                ...
                if score.lib_cosine > best_score {
                    best_score = score.lib_cosine;
                    best_rt = spectrum.retention_time;
                }
            }

            pb.inc(1);

            // Only include if we found any match
            if best_score > 0.0 {
                Some((library_rt, best_rt, best_score, entry.is_decoy))
            } else {
                None
            }
        })
    ...
    pb.finish_with_message("LibCosine scoring complete");
...
fn compute_fdr_and_filter(
    ...
    // Extract features and create scored entries
    ...
    for (lib_id, data_points) in entry_data.iter() {
...
pub fn filter_library_by_detections(
    library: &[LibraryEntry],
...
pub fn get_detected_indices(scored_entries: &[ScoredEntry]) -> Vec<usize> {
...
pub fn run_analysis_with_refinement(
    config: &OspreyConfig,
