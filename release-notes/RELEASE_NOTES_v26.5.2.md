# Osprey v26.5.2 Release Notes

Patch release with two CI / log-hygiene follow-ups to v26.5.1: the per-scan `[unsorted-spectrum]` log added in v26.5.1 is now `debug`-level with a single info-level aggregate summary per file, and the workflow files opt the pinned v4 GitHub Actions into Node 24 ahead of the 2026-06-02 forced switch. No behavioral changes to the analysis pipeline.
